...
- **Self-Healing Experts (自愈专家)**: 核心专家 Agent (质心、拓扑、降维等) 遵循 **Think-Act-Fix** 循环。当代码在执行器中运行报错（如维度不匹配、API 变更）时，Agent 会分析 Traceback 并自动修复代码，上限 3 次重试。
...
- **DataContext 不可变数据上下文**: 数据集以只读 `CTX_DATA` 对象注入执行环境，包含 X, y, n_samples, n_features 等元数据，消除 LLM 生成代码中常见的变量绑定错误。
...
### 3. 受限代码执行 (Restricted Executor)
- **受限执行**: 同进程 `exec()` + builtins 白名单 + 模块黑名单 + 线程超时 + 内存监控。非进程级安全沙箱，LLM 生成代码在 Python 线程内运行。
...
- `agent_core/supervisor.py`: 主控编排器，专家调度与结果聚合（1504 行，此前 3066 行单体已拆分为 4 模块）。
- `agent_core/preflight.py`: 派发前预处理 — 数据结构分类、连通性预检、Hopkins 门禁、规模预算、降维/降采样、流形预处理。
- `agent_core/ranking.py`: 派发后排序 — ARI 一票否决制、共识陷阱检测、图算法交叉验证、拓扑失败检测。
- `agent_core/reflection.py`: 排序后审计 — Critic 审计、Critic 2.0 反馈闭环、LLM-3 反思摘要、最终报告组装。
...
- `tools/coder_sandbox.py`: 受限执行器（同进程 exec + 模块黑名单 + builtins 白名单），DataContext 不可变数据上下文，CORE_PRE_INJECT 预注入。
...
- **测试状态**: 201 项测试（183 benchmark + 18 system），Coverage ~65%（CI 门槛 `--cov-fail-under=30`）。具体通过数因环境而异，以 CI 最新运行为准。
...
- **执行安全**: Phase 0 资源限额（timeout + memory）+ **2026-04-29 高危方法拦截**（os.remove / os.system / sys.exit / subprocess 等封禁）。注意：当前为同进程 `exec()` + 线程超时，非进程级隔离。见 `tools/coder_sandbox.py`。
- **代码规模**: ~8,000 行核心代码（含 benchmark），~3,500 行测试代码（201 项，含 18 项系统集成测试）。
...
- **Human-in-the-Loop (HITL)**：当前为提示词级约束注入（`_inject_constraints_prompt`），非约束优化器（COP-KMeans、must-link/cannot-link）。真正的交互式约束聚类（标注 UI + 约束求解）需单独立项。
...
### 2026-05-22 更新 — supervisor.py 单体拆分

**架构重构：3066 行单体 → 4 模块**
- `agent_core/supervisor.py`: 3066 → 1504 行（-51%），仅保留编排/派发/集成/缓存核心逻辑
- `agent_core/preflight.py` (NEW, ~1100 行): 派发前预处理 — 数据分类、连通性预检、Hopkins/规模门禁、降维、降采样、流形预处理，含 `run_preflight_gates()` 一站式入口
- `agent_core/ranking.py` (NEW, ~350 行): 派发后排序 — ARI 一票否决制、共识陷阱检测、图算法交叉验证、拓扑失败检测
- `agent_core/reflection.py` (NEW, ~480 行): 排序后审计 — Critic 审计执行（3 级超时回退）、Critic 2.0 闭环重试、LLM-3 反思摘要、最终报告组装、错误报告
- 全部旧 `self._xxx()` 方法保留为薄封装器，向后兼容所有消费者（web_demo、测试、demo_runner、ace_template_selector）
- 6 个消费者文件同步更新 `llm_settings=` → `llm_config=MultiLLMConfig(worker=...)`，消除 multi-LLM API 断层
```
cf32b45 docs: update README with ModalityProfile + KnowledgeEngine fixes, add deps
500fbb4 fix: resolve KnowledgeEngine relative-path fragility
f9d1f3f chore: cleanup — remove generated files, update .gitignore, prune dead code
