...
## 日志 #0022 — 2026-06-24 — 决策：离开 BRCA；候选靶清单完成

**类型**：决策 + 研判 · **关联**：`docs/CANDIDATE_TARGETS_2026-06-24.md`、Biology Lesson 13
**PM 决定**：离开饱和的 BRCA 分型，换未饱和、CS 能使上力的问题。

**调研结论（生信 ML 处处拥挤、无空白；比相对开放度+CS 契合+可得性+可快速验证）**：
- **候选 1 药物响应(DepMap/GDSC)**：纯 ML 回归、数据小、8GB 够、迭代最快、最易出真结果；中-高饱和；Q1 ~20-30%。
- **候选 2 空间转录组(ST)**：GNN/表示/多模态、CS 契合最高、最开放、公开数据多；需学 ST 生物学、数据稍重；Q1 ~25-35%。
- **候选 3 计算病理多模态(WSI+omics)**：CV+多模态，但 gigapixel 门槛高、基础模型主导、个人难赢；暂不建议。

**推荐**：候选 1（要最快出结果/低门槛）或候选 2（要最开放/Q1 概率最高）。无论选哪个，**第一步都先验证靶**
（公开数据+最简单基线+看与 SOTA 真实差距），再投入。**待 PM 选靶。**

## 日志 #0023 — 2026-06-24 — 入场空间转录组(ST)；DLPFC 验证：靶有真实开放空间✅

**类型**：决策 + 验证（里程碑，正信号）· **环境**：Tumor_Subtype_Agent（已装 scanpy/anndata/leiden）
**关联**：`st/01_dlpfc_verify.py`、`data/dlpfc/`(DLPFC12.zip 2.1GB + 解出 151673；gitignore)
**PM 选定**：候选 2 空间转录组。

**做了什么**：装 ST 工具栈；下 DLPFC12 基准(Zenodo 10698880, 2.1GB)；解样本 151673
（10x h5 表达 + spatial 坐标 + gt/layered 7 层金标准 L1-6+WM, 3639 spots）；跑非空间 vs 简单空间基线。

**结果（151673, k=7, 3 seeds）**：
| 方法 | ARI | NMI |
|---|---|---|
| 非空间 KMeans/PCA | 0.201 | 0.354 |
| 简单空间(近邻平滑) | **0.317** | 0.482 |
| SOTA 参考 STAGATE~0.60 / GraphST~0.63 | ~0.60 | |

**结论（靶验证通过，与 BRCA 相反）**：
- 空间信息**有用**：朴素近邻平滑即 +0.12 ARI(0.20→0.32) → 机制成立、可利用。
- 距 SOTA **还差 0.28** → **清晰的开放空间**（vs BRCA 跨队列差距仅 0.02、已饱和）。
- 有标准基准 + 明确指标(ARI vs 人工分层) + 大空隙 = **真正可做、可量化取胜的方向**。

**踩坑记**：① `conda run -n ENV python -c "多行"` 报错(newline 不支持)→ 多行用文件或 env python.exe 全路径；
② `unzip "dir/*"` 的 `*` 不跨 `/` → 子目录需显式列出，或用 Python zipfile。

**下一步 ST-W2**：建 **SOTA 级空间 GNN 基线**（图自编码器：空间近邻图 + 表达，GPU），
在 DLPFC 多样本上复现 ~0.5-0.6（="要打败的基线"/L2 内核），再找创新点缩小/超越 SOTA 差距。
诚实：这只是 ST 的起点(靶验证)，真功夫在建方法。

## 日志 #0024 — 2026-06-24 — ST-W2 朴素 GCN-AE 基线：欠 SOTA(架构受限，不粉饰)

**类型**：修改 + 验证（混合）· **环境**：Tumor_Subtype_Agent（GPU 实跑 cuda）· **关联**：`st/02_graph_ae.py`、`st/_stdata.py`
**做了什么**：纯 torch 实现 GCN 图自编码器（空间 kNN 图 Â + 表达 → 图卷积编码/重构 → 潜表示 → GMM/KMeans 聚类），GPU。

**结果（151673, k=7, 3 seeds, GPU）**：
| 方法 | ARI |
|---|---|
| 非空间 KMeans/PCA | 0.201 |
| GCN-AE + KMeans | 0.29 |
| GCN-AE + GMM(mclust风) | **0.336**(lr1e-3) / 0.317(lr1e-4) |
| SOTA 参考 STAGATE~0.60 / GraphST~0.63 | ~0.60 |

**诚实结论**：朴素 GCN-AE 卡在 **~0.33**——超过非空间(0.20)，但**远低于 SOTA(0.60)、甚至不及 SpaGCN(0.45)**。
降 lr 反而略降 → **不是欠调，是架构受限**：SOTA 的增益来自**图注意力(STAGATE)/对比学习(GraphST)** + 设计细节，
不是朴素 GCN。从零手搓朴素 GCN-AE **不能复现 SOTA**。

**纠正路线**：与其继续手搓逼近，不如**装官方 SOTA(GraphST/STAGATE) 当"验证过的基线"**——
...
## 日志 #0026 — 2026-06-24 — ST-W3a 全 12 样本基准 + 失效分析 → 指向多切片整合

**类型**：验证（里程碑）· **环境**：Tumor_Subtype_Agent GPU · **关联**：`st/04_benchmark_all.py`、`st/outputs/benchmark_all.csv`

**12 样本受控基线（GMM+精修, 1 seed）**：
| sample | k | 非空间 | GraphST |
|---|---|---|---|
| 151507 | 7 | 0.200 | **0.598** |
| 151508 | 7 | 0.156 | 0.464 |
| 151509 | 7 | 0.258 | 0.465 |
| 151510 | 7 | 0.183 | 0.375 |
| 151669 | 5 | 0.184 | **0.294** |
| 151670 | 5 | 0.168 | **0.264** |
| 151671 | 5 | 0.254 | 0.523 |
| 151672 | 5 | 0.227 | 0.475 |
| 151673 | 7 | 0.200 | 0.504 |
| 151674 | 7 | 0.214 | 0.452 |
| 151675 | 7 | 0.160 | 0.483 |
| 151676 | 7 | 0.179 | 0.447 |
| **MEAN** | | **0.199** | **0.445** |
（论文 GraphST ~0.63 用 R mclust；我们受控 GMM=0.445，方差大 0.26–0.60。）

**失效分析（指引创新）**：
- 按 donor 均值：donor1(507-510)=0.476、donor2(673-676)=0.472、**donor3(669-672)=0.389(最弱)**。
- donor3 内：**151669=0.294、151670=0.264 崩**，但**相邻 151671=0.523、151672=0.475 不错**。
- → **强烈提示"多切片整合"**：同 donor 的相邻切片是连续组织，弱片可向相邻强片借结构/借力。这是数据结构直接支持的创新假设。

**ST-W3b 决策（候选，多切片证据最强）**：① 多切片整合（弱片借相邻片）——数据结构支持、可做、数据在手；
② 图像多模态（H&E 形态学补表达模糊处）——CS 天花板最高但更重。
**下一步先廉价验证创新假设(不直接建大方法)**：把 donor3 的 4 片**联合**聚类(简单对齐+共图) vs 单片，看 151669/670 是否被救——
验证多切片确有增益再建正式方法(沿用"先验证靶/假设再投入"纪律)。

## 日志 #0027 — 2026-06-24 — 多切片探针：温吞结果(弱支持，非 slam-dunk，不粉饰)

**类型**：验证（创新假设探针）· **环境**：GPU · **关联**：`st/05_multislice_probe.py`
**设计**：donor3 4 片同一联合预处理特征，单片图(各片空间近邻) vs 联合图(空间近邻+跨片 MNN)，同跑 GCN-AE，比各片 ARI。

**结果**：
| 样本 | 单片 | 联合 | GraphST |
|---|---|---|---|
| 151669(弱) | 0.218 | 0.219 | 0.294 |
| 151670(弱) | 0.226 | 0.265 | 0.264 |
| 151671 | 0.257 | 0.400 | 0.523 |
| 151672 | 0.339 | 0.362 | 0.475 |
| MEAN | 0.260 | **0.312(+0.05)** | |

**诚实判读**：
- 多切片**整体温和提升** +0.05（0.260→0.312），151671 提升明显(0.257→0.400)。
- **但"救弱片"这个核心假设只弱支持**：弱片(669+670) 仅 +0.02(<我 0.03 阈值)；最差的 151669 几乎没动(0.218→0.219)——它可能本就是噪数据，借力也救不动。
- 探针用的是我**弱的 GCN-AE**(整体仍不及 GraphST)，测的是"机制"有无信号——信号**温和为正，但非强**。

**结论**：多切片是"maybe"，不是"yes"。**不直接押它建大方法**。两条路待 PM 定：
① 仍开发多切片(在强 backbone 上，赌 +0.05 能转化为超 GraphST)——有风险；
② 廉价**再探图像多模态**(H&E 形态学=正交信息，可能补噪片)——另一强 CS 候选，未验证；
③ 重新审视(也许 donor3 弱片不可救，换更可赢的 ST 子任务)。
诚实：找到真能超 SOTA 的创新本就难；两个探针(多切片温吞)说明要么换角度、要么接受温和增益深耕。

## 日志 #0028 — 2026-06-24 — 图像多模态探针：朴素 ImageNet 有害(用错编码器，非思路错)

**类型**：验证（创新假设探针）· **环境**：GPU · **关联**：`st/06_image_probe.py`
**设计**：每 spot 取 H&E patch → 预训练 ResNet18(ImageNet) 512-d → 与表达 PCA 拼接聚类，比表达-only。

**结果**：
| 样本 | 表达-only | 表达+图像 | GraphST |
|---|---|---|---|
| 151507 | 0.363 | 0.061 | 0.598 |
| 151670 | 0.209 | -0.012 | 0.264 |
| 151673 | 0.290 | 0.211 | 0.504 |
| MEAN | 0.287 | **0.087** | |

**诚实判读**：图像特征**严重拖累**(0.287→0.087)。根因：**ImageNet ResNet18 不是病理编码器**，提的是通用纹理/颜色=对分层是噪声，
高方差还淹没了表达信号。**这不否定"图像多模态"思路，而是探针太糙(编码器用错)**——正确工具是**病理基础模型(UNI/CONCH/Phikon)**，但更重。

**两个创新探针小结(都不亮)**：① 多切片=温吞(+0.05，救弱片没兑现)；② 朴素图像=有害(需病理 FM 才可能可用)。
**没有一个廉价探针给出清晰的超 SOTA 信号。**

**这是个诚实的拐点**(待 PM 定，已用 AskUserQuestion)：
① 用**病理基础模型**做正经图像多模态(更重、有文献支持、未验证)；
② 深耕多切片(赌 +0.05 转化)；
③ 重审 ST 子任务/目标；
④ **坦诚的目标/可行性复位**：BRCA 饱和 + ST 靶真但两探针不亮，个人冲 Q1 极难——是否调成"扎实二区"或重订路线，值得正面谈。
诚实：我不再把温吞/负结果当进展吹；该和 PM 谈清楚现实预期，而不是无限探针。

## 日志 #0029 — 2026-06-24 — 最后一发(病理 FM Phikon)：inconclusive(被损坏 tif 卡住，非干净负)

**类型**：验证（创新假设探针，受阻）· **环境**：GPU · **关联**：`st/07_pathfm_probe.py`、`data/phikon/`(本地权重)
**经过**：HF 被墙→从 hf-mirror curl 下 Phikon(345MB)本地加载；**全分辨率 tif 损坏(tifffile 0 页/PIL 读不了)**→被迫回退糊的 hires PNG patch。

**结果(hires patch, Phikon CLS 768-d + 表达)**：
| 样本 | 表达-only | 表达+Phikon | 表达+ImageNet(#0028) |
|---|---|---|---|
| 151507 | 0.363 | 0.211 | 0.061 |
| 151670 | 0.209 | 0.174 | -0.012 |
| 151673 | 0.290 | **0.364** | 0.211 |
| MEAN | 0.287 | 0.250 | 0.087 |

**诚实判读**：
- **Phikon ≫ ImageNet**(0.250 vs 0.087) → 证实"病理 FM 才是对的编码器"；**151673 真提升**(0.290→0.364)。
- 但整体仍**没稳过表达-only**(0.250<0.287)。
- **关键**：被迫用糊的 hires(spot~14px)——病理 FM 要高清形态学，**这是不公平的测试**。
  → 这一发 **inconclusive(被数据卡住)，不是干净负结果**：有苗头(赢1/3+远超ImageNet)，但公平的全分辨率测试没做成。

**现实抉择(待 PM)**：
① 另寻**可读的全分辨率 DLPFC H&E**(spatialLIBD/10x，Zenodo 这份 tif 坏了)给病理 FM 一次公平测试——更多数据工程、payoff 不确定；
② **复位目标**(二区扎实方法/重订路线)——BRCA 饱和 + ST 三探针(多切片温吞/ImageNet 有害/Phikon 低清不稳)累积说明个人冲一区极难。
**踩坑**：HF 国内需 hf-mirror+本地加载；Zenodo DLPFC12 的 full_image.tif 损坏(0页)不可用。

## 日志 #0030 — 2026-06-24 — 病理 FM 公平测试(全分辨率)：干净的负结果

**类型**：验证（创新假设，公平测试完成）· **环境**：GPU · **关联**：`st/07_pathfm_probe.py`、`data/dlpfc/.../*_full_image.tif`(spatialLIBD S3, 完整 533MB)
**经过**：诊断 Zenodo tif 截断损坏(IFD 偏移 533MB>文件 170MB)→从 **spatialLIBD S3** 下完整 533MB tif(13332×13332)→
Phikon 取 224px 原生高清 patch + 表达，公平对比。

**结果(全分辨率 patch)**：
| 样本 | 表达-only | 表达+Phikon(全分辨率) | (参考)hires |
|---|---|---|---|
| 151507 | 0.363 | 0.189 | 0.211 |
| 151670 | 0.209 | 0.022 | 0.174 |
| 151673 | 0.290 | 0.190 | 0.364 |
| MEAN | **0.287** | **0.134** | 0.250 |

**干净的负结果(公平测试已做)**：全分辨率下 Phikon 多模态**明显更差**(0.134<0.287)，比糊图还差。
**生物学解释(合理)**：**皮层分层由基因表达梯度定义，不是 spot 尺度的 H&E 形态**(神经元密度/大小渐变在小 patch 抓不到)；
表达就是对的模态，拼噪声图像特征只会拖累。224px 原生反而 FOV 太小(只几个细胞、无分层上下文)。

**ST 创新探针全部结清**：① 多切片=温吞(+0.05) ② ImageNet 图像=有害(编码器错) ③ Phikon 图像(公平)=干净负。
**没有找到能超 GraphST 的创新角度。** 最后一发已公平打完，是负的。
